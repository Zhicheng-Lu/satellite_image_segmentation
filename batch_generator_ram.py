...
class Batch_generator_ram:
    ...
    def __init__(self, image_ids, img_rows, img_cols, num_channels, cropping, horizontal_flip=False, vertical_flip=False, swap_axis=False, mask=-1):
        ...
        self.y_train = np.zeros((24, 11, min_train_height, min_train_width))
        #self.X_train = np.zeros((24, 3, min_train_height, min_train_width))
        f = h5py.File('../data/train_3.h5','r')
        #X_train = f['train']
        if mask == -1:
            y_train = np.array(f['train_mask'])

        else:
            y_train = np.array(self.f['train_mask'])[:, mask]
            y_train = np.expand_dims(self.y_train, 1)
        ...
        for i in range(len(image_ids)):
            ...
            self.y_train[:] = self.read_mask(image_id, height, width)[:, :min_train_height, :min_train_width] / 255
            if np.array_equal(self.y_train[i], y_train[i]):
                print('true')
            else:
                print('false')
    ...
    def read_mask(self, image_id, height, width):

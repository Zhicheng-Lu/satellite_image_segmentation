...
def unet(x, keep_prob, channels, n_class, img_rows, img_cols, batch_norm, is_train, blocks=5, layers=2, features_root=32, filter_size=3, pool_size=2, pad="SAME", activation="elu", upsampling=1, initializer="Normal", summaries=True):
    """
    ...
    # whether to share variables (Not in all cases)
    reuse = False
    ...
    with tf.variable_scope("u_net", reuse=reuse):
        ...
        for b in range(blocks):
            for l in range(layers):
                ...
                if batch_norm:
                    data_tensor = BatchNormLayer(data_tensor, act=act_bn, is_train=is_train, gamma_init=gamma_init, name='bn_{0}_{1}'.format(str(b+1), str(l+1)))
        ...
        for b in range(blocks-1):
            ...
            for l in range(layers):
                ...
                if batch_norm:
                    data_tensor = BatchNormLayer(data_tensor, act=act_bn, is_train=is_train, gamma_init=gamma_init, name='ubn_{0}_{1}'.format(str(b+1), str(l+1)))
